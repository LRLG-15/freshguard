...
struct AddFoodView: View {
    ...
    @State private var imageAnalysis: FoodImageAnalysis?
    @State private var showingImageAnalysis = false
    
    // New acquisition date fields
    @State private var boughtToday = true
    @State private var acquiredDate = Date()
    
    // New fresh product AI analysis
    @State private var isFreshProduct = false
    @State private var isOrganic = false
    
    // AI correction states
    @State private var showingNameCorrection = false
    @State private var suggestedName = ""
    @State private var suggestedCategory: FoodCategory?
        ...
        NavigationView {
            Form {
                    ...
                    HStack {
                        ...
                        TextField("Enter food name", text: $foodName)
                            ...
                            .onChange(of: foodName) { newValue in
                                if !newValue.isEmpty {
                                    checkNameWithAI(newValue)
                                }
                            }
                    }
                    
                    if showingNameCorrection {
                        HStack {
                            Image(systemName: "lightbulb.fill")
                                .foregroundColor(.yellow)
                            VStack(alignment: .leading) {
                                Text("Did you mean: \(suggestedName)?")
                                    .font(.caption)
                            ...
                            HStack {
                                ForEach(foodEmojis, id: \.self) { emoji in
                                    Text(emoji)
                                        .font(.title2)
                                        .padding(8)
                                        .background(selectedEmoji == emoji ? Color.pastelBlue.opacity(0.3) : Color.clear)
                                        .cornerRadius(8)
                                        .onTapGesture {
                                            selectedEmoji = emoji
                                        }
                ...
                Section("Acquisition Date") {
                    ...
                    if !boughtToday {
                        DatePicker("Date Acquired", selection: $acquiredDate, displayedComponents: .date)
                    }
                }
                ...
                Section("Expiration") {
                    ...
                    if hasExpirationDate && !isFreshProduct {
                        DatePicker("Expires on", selection: $expirationDate, displayedComponents: .date)
                    }
                    
                    if isFreshProduct {
                        Text("AI will calculate optimal expiration date based on product type and freshness")
                            .font(.caption)
                        ...
                        if let analysis = imageAnalysis {
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                if !analysis.detectedItems.isEmpty {
                                    Text("Detected: \(analysis.detectedItems.joined(separator: ", "))")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                                }
                            }
    ...
    private func checkNameWithAI(_ name: String) {
        let analysis = aiManager.analyzeFoodItemName(input: name)
        
        if analysis.confidence < 1.0 && analysis.correctedName != name {
            suggestedName = analysis.correctedName
            suggestedCategory = analysis.suggestedCategory
            showingNameCorrection = true
        }
    }
    
    private func analyzeImage(_ image: UIImage) {
        aiManager.analyzeFoodImage(image: image) { analysis in
    ...
    private func addFood() {
        // Get AI analysis for name correction
        let nameAnalysis = aiManager.analyzeFoodItemName(input: foodName)
        let finalName = nameAnalysis.correctedName
        let finalCategory = nameAnalysis.suggestedCategory ?? selectedCategory
        
        // Calculate expiration date
        let finalExpirationDate: Date?
        if isFreshProduct {
            finalExpirationDate = aiManager.calculateSmartExpiry(
                for: finalCategory,
                acquiredDate: boughtToday ? Date() : acquiredDate,
                isOrganic: isOrganic
            )
        } else if hasExpirationDate {
            finalExpirationDate = expirationDate
        } else {
            finalExpirationDate = nil
        }
        ...
        foodManager.addFoodItem(newItem)
        
        // Send notification
        NotificationCenter.default.post(
            name: .foodItemAdded,
            object: nil,
            userInfo: ["foodName": finalName]
        )
